...
- The landing page renders, in order: the intro (`docs/src/intro.md`), a
  hand-curated Featured `CardGrid`, an auto-generated Repositories list
  (`docs/src/data/repos.json`), and org links. `intro.md` and `repos.json`
  are generated by `just docs-repos` (see below) and committed so builds need
  no network.
